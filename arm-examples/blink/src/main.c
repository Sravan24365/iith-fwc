...
#define X_PIN 22 //red led
#define Y_PIN   18      //blue led
#define F_PIN   21      //green led
int main(void)
{
    uint32_t i=0,j=0,k=0;
    uint32_t C=0,B=0,A=0;
    uint32_t X=0,Y=0,F=0;    
...
PyHal_GPIO_SetDir(C_PIN,0); //Input

PyHal_GPIO_SetDir(X_PIN,1); //Output
PyHal_GPIO_SetDir(Y_PIN,1); //Output
PyHal_GPIO_SetDir(F_PIN,1); //Output
...
while(1)
{
    ...
    PyHal_GPIO_Set(X_PIN, X);
    PyHal_GPIO_Set(Y_PIN, Y);
    PyHal_GPIO_Set(F_PIN, F);
   
    
    A = PyHal_GPIO_Get(A_PIN);
    B = PyHal_GPIO_Get(B_PIN);
    C = PyHal_GPIO_Get(C_PIN);
